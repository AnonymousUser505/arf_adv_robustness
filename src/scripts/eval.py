...
from src.datasets.utils import get_dataset_inds, get_mini_dataset_inds, get_ensemble_dir, \
    get_dump_dir, get_boundary_val_test_inds, get_attack_inds
from src.utils import set_logger, get_ensemble_paths, majority_vote, convert_tensor_to_image, calc_attack_rate, \
    get_image_shape, print_norm_dists, dump_imgs_to_dir
from src.models.utils import get_strides, get_conv1_params, get_model
from src.classifiers.pytorch_classifier_specific import PyTorchClassifierSpecific
...
parser.add_argument('--method', default='random_forest', type=str, 
                    help='simple, ensemble, tta, tta_counter, random_forest, logistic_regression, svm_linear, svm_rbf')
parser.add_argument('--attack_dir', default='fgsm2', type=str, help='attack directory, or '' for normal images')
...
if is_attacked:
    x_inds, gt_inds = get_attack_inds(dataset, attack_args['attack'])
else:
    x_inds = None
    gt_inds = test_inds
